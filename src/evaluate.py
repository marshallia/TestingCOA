...
params = yaml.safe_load(
    open(
        os.path.join(BASE_PATH, "params.yaml"))
)["evaluate"]
batch_size = params["batch_size"]

test_feature = os.path.join(BASE_PATH,
                            sys.argv[2],
                            "bottleneck_features_test.npy")
test_label_feature = os.path.join(BASE_PATH,
                                  sys.argv[2],
        ...
        json.dump(
            {
                name: [
                    {keys[0]: str(val1),
                     ...
                     keys[2]: str(val3)}
                    for val1, val2, val3
                    in zip(list_val1, list_val2, thresholds)
                ]
            },
            file,
            indent=4,
